...
export const roleHierarchy = { [Role.SUPERADMIN]: 4, [Role.ADMIN]: 3, [Role.EDITOR]: 2, [Role.USER]: 1 } as const;
...
export const isAdmin = (role: Role): boolean => role === Role.ADMIN || role === Role.SUPERADMIN;
export const isAdminOrAbove = (role: string | null | undefined): boolean => role === Role.ADMIN || role === Role.SUPERADMIN;
...
export const getRoleDisplayName = (role: Role): string => ({ [Role.SUPERADMIN]: 'Super Administrator', [Role.ADMIN]: 'Administrator', [Role.EDITOR]: 'Editor', [Role.USER]: 'User' })[role] || 'Unknown';
